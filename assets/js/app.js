...
app.torrent_socket.onmessage = function (event) {
    ...
    for (var i = 0; i < torrents.length; i++) {
        ...
        if (app.open_model_id == torr.id) {
            ...
            updateTorrentFiles(torr.files, torr.name, torr.id);
        }
...
function updateTorrentFiles(files, id, name) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        var row = `<li class="flex items-center justify-between py-2.5">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <button type="button" onclick="downloadFile('${file.path},${name},${id}')"
                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-5 h-5 mr-1 bi bi-cloud-download" viewBox="0 0 16 16" onclick="Alert('TODO: Play Video', 'warn')">
...
function downloadFile(path, name, id) {
    window.location.href = "/api/torrents/files/download?file_path=" + path + "&torrent_name=" + name + "&id=" + id;
}
